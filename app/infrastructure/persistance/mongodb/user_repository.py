...
class UserRepositoryImpl(UserRepository):
    ...
    def _to_doc(self, dao: UserDao) -> dict:
        return dao.to_encrypted_document(self._encrypt)

    def _from_doc(self, document: dict) -> UserDao:
        return UserDao.from_encrypted_document(document, self._decrypt)
    ...
    async def get_by_id(self, user_id: UUID) -> Optional[User]:
        ...
        if document is None:
            return None
        return from_dao_to_user(self._from_doc(document))
    ...
    async def get_by_group(self, group: str) -> List[User]:
        ...
        return [from_dao_to_user(self._from_doc(d)) for d in documents]
